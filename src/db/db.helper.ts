...
export default class CustomDbModel{
   ...
   //Adding new Student row
    async addStudentToFile(student: StudentDTO): Promise<Student>{    
        const newstudent = new Student( new Date().getTime(),student.name,student.gender,student.phone,student.email,student.nationality,student.dob ,student.educationbackground,student.preferredmodeofcontact);
        this.appendToFile(JSON.stringify(newstudent))
        return new Student( newstudent.id,newstudent.name,newstudent.gender,newstudent.phone,newstudent.email,newstudent.nationality,newstudent.dob,newstudent.educationbackground,newstudent.preferredmodeofcontact);
    }

    //Get a list of students
    async getListOfStudents(getStudents : GetStudentsDTO): Promise<Array<Student>>{
